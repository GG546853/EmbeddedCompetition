...
static uint16_t tp_read_ad(uint8_t cmd)
{
    ...
    sys_delay_us(6);        /* ADS7846的转换时间最长为6us */
    T_CLK(0);
    sys_delay_us(1);
...
{
    rgblcd_draw_line(x - 12, y, x + 13, y, color);  /* 横线 */
    rgblcd_draw_line(x, y - 12, x, y + 13, color);  /* 竖线 */
    rgblcd_draw_point(x + 1, y + 1, color);
    rgblcd_draw_point(x - 1, y + 1, color);
    rgblcd_draw_point(x + 1, y - 1, color);
    rgblcd_draw_point(x - 1, y - 1, color);
    rgblcd_draw_circle(x, y, 6, color);             /* 画中心圈 */
}
...
{
    rgblcd_draw_point(x, y, color);         /* 中心点 */
    rgblcd_draw_point(x + 1, y, color);
    rgblcd_draw_point(x, y + 1, color);
    rgblcd_draw_point(x + 1, y + 1, color);
}
...
static uint8_t tp_scan(uint8_t mode)
{
    if (T_PEN == 0)     /* 有按键按下 */
    {
        ...
        else if (tp_read_xy2(&tp_dev.x[0], &tp_dev.y[0]))     /* 读取屏幕坐标, 需要转换 */
        {
            ...
            tp_dev.x[0] = (signed short)(tp_dev.x[0] - tp_dev.xc) / tp_dev.xfac + rgblcddev.width / 2;

            /* 将Y轴 物理坐标转换成逻辑坐标(即对应LCD屏幕上面的Y坐标值) */
            tp_dev.y[0] = (signed short)(tp_dev.y[0] - tp_dev.yc) / tp_dev.yfac + rgblcddev.height / 2;
        }
...
void tp_save_adjust_data(void)
{
    ...
    data[0] = 0x0A;
    eeprom_write(TP_SAVE_ADDR_BASE + 12, data, 1);  /* 保存校准值 */
}
...
uint8_t tp_get_adjust_data(void)
{
    ...
    eeprom_read(TP_SAVE_ADDR_BASE + 12, data, 1);   /* 读取校准状态标记 */
...
static void tp_adjust_info_show(uint16_t xy[5][2], double px, double py)
{
    ...
    for (i = 0; i < 5; i++)   /* 显示5个物理坐标值 */
    {
        ...
        rgblcd_show_string(40, 160 + (i * 20), rgblcddev.width, rgblcddev.height, 16, sbuf, RED);
        sprintf(sbuf, "y%d:%d", i + 1, xy[i][1]);
        rgblcd_show_string(40 + 80, 160 + (i * 20), rgblcddev.width, rgblcddev.height, 16, sbuf, RED);
    }

    /* 显示X/Y方向的比例因子 */
    rgblcd_fill(40, 160 + (i * 20), rgblcddev.width - 1, 16, WHITE);   /* 清除之前的px,py显示 */
...
void tp_adjust(void)
{
    ...
    rgblcd_clear(WHITE);    /* 清屏 */
    rgblcd_show_string(40, 40, 160, 100, 16, TP_REMIND_MSG_TBL, RED);   /* 显示提示信息 */
        ...
        if ((tp_dev.sta & 0xc000) == TP_CATH_PRES)   /* 按键按下了一次(此时按键松开了.) */
        {
            ...
            switch (cnt)
            {
                case 1:
                    ...
                    tp_draw_touch_point(rgblcddev.width - 20, 20, RED);    /* 画点2 */
                    break;

                case 2:
                    tp_draw_touch_point(rgblcddev.width - 20, 20, WHITE);  /* 清除点2 */
                    tp_draw_touch_point(20, rgblcddev.height - 20, RED);   /* 画点3 */
                    break;

                case 3:
                    tp_draw_touch_point(20, rgblcddev.height - 20, WHITE); /* 清除点3 */
                    tp_draw_touch_point(rgblcddev.width - 20, rgblcddev.height - 20, RED);    /* 画点4 */
                    break;

                case 4:
                    rgblcd_clear(WHITE); /* 画第五个点了, 直接清屏 */
                    tp_draw_touch_point(rgblcddev.width / 2, rgblcddev.height / 2, RED);  /* 画点5 */
                    break;
                ...
                case 5:     /* 全部5个点已经得到 */
                    ...
                    {
                        cnt = 0;
                        tp_draw_touch_point(rgblcddev.width / 2, rgblcddev.height / 2, WHITE); /* 清除点5 */
                        tp_draw_touch_point(20, 20, RED);   /* 重新画点1 */
                        tp_adjust_info_show(pxy, px, py);   /* 显示当前信息,方便找问题 */
                        continue;
                    }

                    tp_dev.xfac = (float)(s1 + s3) / (2 * (rgblcddev.width - 40));
                    tp_dev.yfac = (float)(s2 + s4) / (2 * (rgblcddev.height - 40));

                    tp_dev.xc = pxy[4][0];      /* X轴,物理中心坐标 */
                    tp_dev.yc = pxy[4][1];      /* Y轴,物理中心坐标 */

                    rgblcd_clear(WHITE);   /* 清屏 */
                    rgblcd_show_string(35, 110, rgblcddev.width, rgblcddev.height, 16, "Touch Screen Adjust OK!", BLUE); /* 校正完成 */
                    HAL_Delay(1000);
                    tp_save_adjust_data();
...
uint8_t tp_init(void)
{
    ...
    tp_dev.touchtype |= rgblcddev.dir & 0X01;  /* 根据LCD判定是横屏还是竖屏 */

    if (rgblcddev.id == 0x7796)                /* 3.5寸屏有两种，一种屏幕ID为0x5510带电阻触摸屏，一种屏幕ID为0x7796带GT型号的电容触摸屏 */
    {
    ...
    if (rgblcddev.id == 0X5510 || rgblcddev.id == 0X9806 || rgblcddev.id == 0X4342 || rgblcddev.id == 0X4384 || rgblcddev.id == 0X1018)  /* 电容触摸屏,4.3寸/10.1寸屏 */
    {
        gt9xxx_init();
        tp_dev.scan = gt9xxx_scan;      /* 扫描函数指向GT9147触摸屏扫描 */
        tp_dev.touchtype |= 0X80;       /* 电容屏 */
    ...
    else if (rgblcddev.id == 0X1963 || rgblcddev.id == 0X7084 || rgblcddev.id == 0X7016)     /* SSD1963 7寸屏或者 7寸800*480/1024*600 RGB屏 */
    {
